...
export default function Home() {
  ...
  return (
    <main className="flex min-h-screen flex-col items-center p-4 md:p-6 bg-black text-white pb-20">
      <div className="w-full max-w-xl space-y-6">
        <div className="flex items-center justify-between border-b border-zinc-900 pb-4">
          ...
          <button
            ...
            className="flex items-center gap-1.5 px-3 py-1.5 rounded-xl bg-zinc-900 border border-zinc-800 text-xs text-zinc-400 hover:text-red-400 transition"
            title="تبديل الاسم"
          >
            ...
            selectedExercises.map((exercise) => (
              ...
              >
                <div className="flex items-center justify-between">
                  ...
                  <div className="flex items-center gap-2">
                    <button
                      ...
                      className="p-2 rounded-xl bg-zinc-900 border border-zinc-800 text-amber-400 hover:bg-zinc-800 transition"
                    ...
                    <button
                      ...
                      className="p-2 rounded-xl bg-red-950/20 border border-red-900/30 text-red-400 hover:bg-red-950/40 transition"
                      title="حذف التمرين"
                    >
                          ...
                          <div className="flex flex-wrap gap-1.5">
                            {workout.sets.map((s, sIdx) => (
                              <span key={s.id || sIdx} className="bg-zinc-950 px-2 py-0.5 rounded text-zinc-300 border border-zinc-900">
                                S{sIdx + 1}: <strong className="text-white">{s.weight || 0}كغ</strong> × <strong className="text-white">{s.reps || 0}تكرار</strong>
                              </span>
                            ))}
                          </div>
                        </div>
                ...
                <div className="space-y-2 pt-2 border-t border-zinc-900">
                  <div className="grid grid-cols-12 gap-2 text-[11px] font-medium text-zinc-500 px-1">
                    <div className="col-span-2 text-center">SET</div>
                    <div className="col-span-4 text-center">KG (الوزن)</div>
                    <div className="col-span-4 text-center">REPS (التكرارات)</div>
                    ...
                    <div key={set.id} className="grid grid-cols-12 gap-2 items-center">
                      ...
                      <div className="col-span-4">
                        <input
                          ...
                          className="w-full bg-zinc-900 border border-zinc-800 text-center rounded-xl py-2.5 text-sm text-zinc-100 outline-none focus:border-zinc-700"
                        />
                      </div>
                      ...
                      <div className="col-span-2 flex justify-center">
                        {(exercise.sets ?? []).length > 1 && (
                          <button
                            ...
                            className="text-zinc-600 hover:text-red-400 p-2 transition text-xs font-bold"
                          >
                            ✕
                          </button>
                        )}
                  ...
                  <button
                    ...
                    className="w-full mt-2 py-2.5 rounded-xl border border-dashed border-zinc-800 bg-zinc-900/30 text-xs font-medium text-zinc-400 hover:text-zinc-200 hover:bg-zinc-900 hover:border-zinc-700 transition flex items-center justify-center gap-1.5"
                  >
      ...
      {/* نافذة الرسم البياني (Graph Modal) */}
      {activeGraphExercise && (
        <div className="fixed inset-0 bg-black/80 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="w-full max-w-md rounded-3xl border border-zinc-800 bg-zinc-950 p-6 space-y-5 shadow-2xl animate-fade-in">
            ...
            <div className="space-y-3 max-h-[60vh] overflow-y-auto pr-1">
              {activeGraphExercise.history.length === 0 ? (
                <div className="py-10 text-center text-xs text-zinc-500">
                  لا توجد سجلات سابقة لهذا التمرين لعرضها بيضياً. ابدأ بتسجيل الأوزان لتحصل على تتبع دقيق!
                </div>
              ) : (
            ...
            <button
              onClick={() => setActiveGraphExercise(null)}
              className="w-full py-2.5 rounded-xl bg-zinc-900 border border-zinc-800 text-xs font-medium text-zinc-300 hover:bg-zinc-800 transition"
            >
              إغلاق
            </button>
          </div>
